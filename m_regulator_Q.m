if 1
    ...
    if iter<=ilosc_probek_sterowanie_reczne
        ...
        if uczenie==1 && pozwolenie_na_uczenia==1 && stan_T0 ~= 0 && old_stan_T0 ~= 0
            % Q_update=alfa*(R+gamma*maxS-Q_2d(old_state,wyb_akcja));
            % Q_2d(old_state,wyb_akcja)=Q_2d(old_state,wyb_akcja) + Q_update;
    ...
    if (stan == nr_stanu_doc)
        ...
        if eps>=a
            %stara metoda
            % if eps>=a && wyb_akcja>wyb_akcja_under-RD &&...
            %         wyb_akcja>wyb_akcja_above-RD &&...
            %         wyb_akcja<wyb_akcja_under+RD &&...
            %         wyb_akcja<wyb_akcja_above+RD &&...
            %         wyb_akcja~=nr_akcji_doc
    ...
    %koniec zmiany
    if eks_wer==0
        realizacja_traj_epoka(end+1)=R;
    end

    wart_akcji_bez_f_rzutujacej=wart_akcji;

    if f_rzutujaca_on==1 && (stan ~= nr_stanu_doc && stan ~= nr_stanu_doc+1 && stan ~= nr_stanu_doc-1 && abs(e)>=dokladnosc_gen_stanu)
        % if f_rzutujaca_on==1 && (stan ~= nr_stanu_doc && abs(e)>=dokladnosc_gen_stanu)
        % if f_rzutujaca_on==1 && stan ~= nr_stanu_doc
        % if f_rzutujaca_on==1 && (stan ~= nr_stanu_doc && abs(e)>dokladnosc_gen_stanu && ((wart_akcji<0 && wart_akcji - funkcja_rzutujaca<0) || (wart_akcji>0 && wart_akcji - funkcja_rzutujaca>0)))
        funkcja_rzutujaca=(e*(1/Te-1/Ti));
        %     funkcja_rzutujaca=(e*(1/Te-1/Ti))*(1-epoka/1000);
