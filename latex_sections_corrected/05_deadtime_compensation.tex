...
\end{equation}
%
where $\lfloor \cdot \rfloor$ denotes the floor function. For example, with $T_{0,\text{controller}} = 4$~s and $T_s = 0.1$~s, the buffer holds $N_{\text{buffer}} = 40$ elements.
...
\begin{algorithmic}[1]
\REQUIRE Current state $s_t$, action $a_t$, learning flag $L_t$
...
\IF{$T_{0,\text{controller}} > 0$}
    ...
    \IF{$s_{\text{delayed}} = i_{\text{goal}}$ \AND $a_{\text{delayed}} = a_{\text{goal}}$}
        \STATE $s_{\text{bootstrap}} \gets i_{\text{goal}}$
    \ELSE
...
\ELSE
    ...
    \STATE
    \STATE \COMMENT{Step 1: Use previous state-action for update}
    \STATE $s_{\text{prev}} \gets s_{t-1}$
    \STATE $a_{\text{prev}} \gets a_{t-1}$
    \STATE $L_{\text{prev}} \gets L_{t-1}$
    \STATE
    \STATE \COMMENT{Step 2: Current state is next state}
    \STATE $s_{\text{next}} \gets s_t$
    \STATE
