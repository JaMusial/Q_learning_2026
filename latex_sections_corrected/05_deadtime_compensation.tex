% ========================================================================
% SECTION 4: DEAD TIME COMPENSATION (CORRECTED VERSION)
% ========================================================================
% Changes:
% - First person ("we introduce", "we implement")
% - Uppercase symbols Y(t), U(t), lowercase e(t) for error
% - No made-up convergence values
% - Algorithm matches actual implementation
% - Updated: Error symbol changed from E to e throughout
% - Updated: Buffer size includes +1 for timing alignment
% - Updated: Bootstrap override for numerical stability
% - Updated: Reward logic for goal state maintenance
% ========================================================================
...
When dead time exists, this assumption breaks. For example, with $T_0 = 4$~s and $T_s = 0.1$~s, the plant output at time $t$ reflects the control action applied 40 time steps earlier, not the action $a_t$ just selected. Applying~\eqref{eq:standard_q_update_timestep} would incorrectly associate $s_{t+1}$ with $a_t$, leading to poor or failed learning.
...
%
\begin{equation}
N_{\text{buffer}} = \text{round}\left( \frac{T_{0,\text{controller}}}{T_s} \right) + 1
...
Algorithm~\ref{alg:delayed_credit_assignment} presents our delayed credit assignment procedure.
...
\IF{$T_{0,\text{controller}} > 0$}
    ...
    \STATE
    \STATE \COMMENT{Step 2: Current state is "next state" for delayed action}
    \STATE $s_{\text{next}} \gets s_t$
    \STATE
    \STATE \COMMENT{Step 3: Reward assignment (goal arrival OR goal maintenance)}
    \IF{$s_{\text{next}} = i_{\text{goal}}$ \OR ($s_{\text{delayed}} = i_{\text{goal}}$ \AND $a_{\text{delayed}} = a_{\text{goal}}$)}
        \STATE $R \gets 1$
    \ELSE
        \STATE $R \gets 0$
    \ENDIF
    \STATE
    \STATE \COMMENT{Step 4: Bootstrap state with goal override for numerical stability}
    \IF{$s_{\text{delayed}} = i_{\text{goal}}$ \AND $a_{\text{delayed}} = a_{\text{goal}}$}
        \STATE $s_{\text{bootstrap}} \gets i_{\text{goal}}$ \COMMENT{Override to prevent drift}
    \ELSE
        \STATE $s_{\text{bootstrap}} \gets s_{\text{next}}$
    \ENDIF
    \STATE
    ...
    \ENDIF
\ELSE
    \STATE \COMMENT{No Compensation Mode (One-Step Q-Learning)}
    \STATE
    \STATE \COMMENT{Step 1: Use previous iteration's state-action-reward}
    \STATE $s_{\text{prev}} \gets s_{t-1}$; \quad $a_{\text{prev}} \gets a_{t-1}$
    \STATE $L_{\text{prev}} \gets L_{t-1}$; \quad $R_{\text{prev}} \gets R_{t-1}$
    \STATE
    \STATE \COMMENT{Step 2: Current state is next state for previous pair}
    \STATE $s_{\text{next}} \gets s_t$
    \STATE
    \STATE \COMMENT{Step 3: Q-learning update using previous reward}
    \IF{$L_{\text{prev}} = 1$}
        \STATE $Q(s_{\text{prev}}, a_{\text{prev}}) \gets Q(s_{\text{prev}}, a_{\text{prev}})$
        \STATE \hspace{2em} $+ \alpha \left[R_{\text{prev}} + \gamma \max_{a'} Q(s_{\text{next}}, a') - Q(s_{\text{prev}}, a_{\text{prev}})\right]$
    \ENDIF
\ENDIF
\end{algorithmic}
\end{algorithm}

\subsubsection{Reward Logic for Goal State Maintenance}

The reward assignment in Algorithm~\ref{alg:delayed_credit_assignment} includes two conditions that grant positive reward:

\noindent\textbf{Goal Arrival:} $R = 1$ when $s_{\text{next}} = i_{\text{goal}}$, rewarding transitions that successfully reach the goal state.

\noindent\textbf{Goal Maintenance:} $R = 1$ when $s_{\text{delayed}} = i_{\text{goal}}$ and $a_{\text{delayed}} = a_{\text{goal}}$, ensuring that the state-action pair $(i_{\text{goal}}, a_{\text{goal}})$ continues to receive positive reward even when disturbances cause temporary deviations from the goal.
...
\subsubsection{Bootstrap Override for Numerical Stability}

When the buffered state-action pair corresponds to the goal state with goal action, the bootstrap state is overridden to the goal state regardless of the actual next state. This prevents numerical drift over the buffer delay from contaminating the Q-value updates. Small discretization errors accumulated over $N_{\text{buffer}}$ time steps can cause the observed next state to be adjacent to (rather than exactly at) the goal state. Without the override, this would cause $Q(i_{\text{goal}}, a_{\text{goal}})$ to bootstrap from a lower-valued adjacent state, inappropriately reducing its value over time.
...
\item \textbf{Time $t$ (plant):} Control $U(t)$ enters the plant's dead time buffer of length $T_0/T_s$
\item \textbf{Time $t$ (controller):} State-action pair $(s(t), a(t))$ enters controller's buffer of length $T_{0,\text{controller}}/T_s$
...
During the manual control phase (first 5--10 samples), we pre-fill both plant and controller buffers with appropriate values:
\begin{itemize}
\item Plant buffers: Filled with $U = Y_{sp}/k$ (steady-state control)
...
\begin{equation}
R = \begin{cases}
1 & \text{if } s_{\text{next}} = s_{\text{goal}} \text{ (goal arrival)} \\
1 & \text{if } s = s_{\text{goal}} \text{ and } a = a_{\text{goal}} \text{ (goal maintenance)} \\
0 & \text{otherwise}
\end{cases}
\label{eq:sparse_reward}
\end{equation}
%
where $s_{\text{goal}}$ is the discrete state closest to $s = 0$, $a_{\text{goal}}$ is the zero control increment action, and $s_{\text{next}}$ is the observed next state after taking action $a$ in state $s$.
...
Under the sparse reward strategy, the Q-value at the goal state asymptotically approaches:
%
\begin{equation}
...
We maintain buffers across episode boundaries:
\begin{itemize}
