function [val_action, num_action] = f_best_action_in_state(table, state, nr_akcji_doc)
...
% Handle ties — pick the one closest to nr_akcji_doc (goal action)
same_mask = (row == val_action);
same_count = nnz(same_mask);

if same_count > 1
    candidates = find(same_mask);
    [~, idx_min] = min(abs(candidates - nr_akcji_doc));
